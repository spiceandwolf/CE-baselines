...
if __name__ == '__main__':
    ...
    with open(workload_in_path, 'r') as read_files:
        for line in read_files:
            ...
            sql, true_card = split_infos[0], split_infos[1]
            sqls.append(sql)
    ...
    for i, sql in enumerate(sqls):
        ...
        pg_card = result.subquery_2_card[subqueries[-1]]
        query = sql + f"||{true_cards[i]}||{pg_card}||@{i}-{count}@"
        ...
        for j, subquery in enumerate(subqueries):
            count += 1
        ...
        assert int(true_cards[i]) == subquery_true_card, f"sql {sql} \n vs {subquery} \n, true_card: {true_cards[i]}, subquery_true_card: {subquery_true_card}"
